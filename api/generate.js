...
module.exports = async function handler(req, res) {
  ...
  const { topic, genre, moods, structure, tempo, rhyme, pov, words, styleNotes } = body;
  if (!topic) return res.status(400).json({ error: 'topic is required' });

  const moodStr = Array.isArray(moods) ? moods.join(', ') : (moods || 'Unspecified');

  const lyricsSys = `You are an expert songwriter with deep knowledge of multiple genres. You write authentic, emotionally resonant lyrics that sound like they came from a real artist. You understand meter, rhyme, imagery, and storytelling. Always label each section clearly — VERSE 1, CHORUS, BRIDGE etc. Never add explanation or commentary — just the lyrics.`;
...
Write the complete lyrics now. Label every section. Make it authentic.`;
  ...
  const titlesUsr = `Suggest 3 song titles for a ${genre || 'general'} song about: ${topic}`;
